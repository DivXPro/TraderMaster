...
export const PREDICTION_PRICE_HEIGHT = 10; // 预测价格高度（价格单位）
export const PREDICTION_GENERATION_INTERVAL = 30; // 预测生成间隔（秒）（若不重叠则匹配持续时间，若重叠则更小）
export const PREDICTION_LAYERS = 6; // 预测层数（上下层数）
export const PREDICTION_INITIAL_COLUMNS = 8; // 初始预生成列数（覆盖图表右侧区域）
